...
Node.jsのバージョンが低いと正常に動作しない場合があります。その場合は、確認済みのバージョンにしてください。
...
`$ npm run build`で出力された`dist/`を提出してください。以下のような処理が行われます。
...
# CSS
## SCSS(Dart Sass)
フォルダ構成、命名規則、Dart Sassでの注意点などは `README-CSS.md` に記載しています。

## ソースマップ
CSSにソースマップを出力するかの設定はwebpackの設定ファイル（webpack.dev.jsなど）で記載する `devtool` に依存します。<br>
ソースマップを出力しない場合は `devtool` を行ごと削除してください。

## Tailwind CSS
[Tailwind CSS](https://tailwindcss.com/)の設定方法です。

### package.json
`"devDependencies"`に`tailwindcss`を追加します。

### tailwind.config.js
Tailwind CSSの設定ファイルです。`webpack.prod.js`等と同じ階層に配置してください。
```
...
**webpack.local.js**  
**webpack.prod.js**  
`postcssOptions: { plugins[] }` に下記を追加します。  
```
require('tailwindcss')
```
...
1. `/scss/`に`tailwind`フォルダを作成します。
2. `tailwind`フォルダに`_.scss`を作成します。
...
3. `/scss/styles.scss`で読み込みます。読み込む場所は一番下です。
...
### メディアクエリの追加
tailwindでメディアクエリを追加する設定です。  
`tailwind.config.js`に下記の設定をします。
...
# サポートブラウザ
サポートするブラウザは[Browserslist](https://browsersl.ist/)の「defaults」設定です。  
Browserslistではdefaultsやブラウザシェアを調べることができます。
